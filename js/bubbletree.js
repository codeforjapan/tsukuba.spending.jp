...
var BubbleTree = function (config, onHover, onUnHover) {
  ...
  me.traverseBubbles = function (parentBubble) {
    ...
    if (children.length > 0) {
      // create ring
      ring = me.createRing(parentBubble.node, parentBubble.pos, 0, {
        stroke: '#888',
        'stroke-dasharray': "-"
      });
    }
  ...
  */
  ...
  me.initHistory = function () {
    $.history.init(me.urlChanged.bind(me), {
      unescape: ",/"
    });
  };
...
BubbleTree.Bubbles.Plain = function (node, bubblechart, origin, radius, angle, color) {
  ...
  me.onhover = function (e) {
    ...
    e.mousePos = {
      x: e.origEvent.pageX - c.offsetLeft,
      y: e.origEvent.pageY - c.offsetTop
    };
    e.type = 'SHOW';
    me.bc.tooltip(e);
  };
  ...
  me.draw = function () {
    ...
    me.circle.attr({
      ...
      cy: me.pos.y,
      r: r,
      'fill-opacity': me.alpha
    });
    if (me.node.children.length > 1) me.dashedBorder.attr({
      cx: me.pos.x,
      cy: me.pos.y,
      r: r - 4,
      'stroke-opacity': me.alpha * 0.9
    });
    else me.dashedBorder.attr({
      'stroke-opacity': 0
    });
    ...
    me.label.css({
      left: (me.pos.x - r) + 'px',
      top: (me.pos.y - me.label.height() * 0.5) + 'px'
    });
...
BubbleTree.Bubbles.Donut = function (node, bubblechart, origin, radius, angle, color) {
  ...
  this.draw = function () {
    ...
    me.label.css({
      left: (me.pos.x - r * 0.9) + 'px',
      top: (me.pos.y - me.label.height() * 0.53) + 'px'
    });
    ...
    if (me.breakdown.length > 1) {
      ...
      for (i in me.breakdown) {
        var col = me.breakdownColors[i] ? me.breakdownColors[i] : '#fff',
          ...
          .attr({
            fill: col,
            'fill-opacity': Math.random() * 0.4 + 0.3,
            stroke: '#fff'
          });
...
BubbleTree.Bubbles.Icon = function (node, bubblechart, origin, radius, angle, color) {
  ...
  me.draw = function () {
    ...
    me.label.css({
      width: (showIcon ? r * 1.2 : 2 * r) + 'px',
      opacity: me.alpha
    });
    me.label.css({
      left: (showIcon ? x - r * 0.6 : x - r) + 'px',
      top: ly + 'px'
    });
